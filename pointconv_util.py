...
class ConvGRU(nn.Module):
    def __init__(self, hidden_dim=128, input_dim=192+128):
        ...
        self.convz = Conv1d(hidden_dim+input_dim, hidden_dim, 3, padding=1)
        self.convr = Conv1d(hidden_dim+input_dim, hidden_dim, 3, padding=1)
        self.convq = Conv1d(hidden_dim+input_dim, hidden_dim, 3, padding=1)
...
class SceneFlowEstimatorResidualIter(nn.Module):

    def __init__(self, feat_ch, cost_ch, flow_ch = 3, channels = [64, 64],  neighbors = 9, clamp = [-200, 200], use_leaky = True, weightnet=16):
        super(SceneFlowEstimatorResidualIter, self).__init__()
        self.clamp = clamp
        self.use_leaky = use_leaky
        self.pointconv_list = nn.ModuleList()
        self.mlp = [feat_ch, feat_ch]
        last_channel = feat_ch + cost_ch
        ...
        self.mlp_convs = nn.ModuleList()
        for _, ch_out in enumerate(self.mlp):
            self.mlp_convs.append(Conv1d(last_channel, ch_out))
            last_channel = ch_out

        self.fc = nn.Conv1d(last_channel, 3, 1)

    def forward(self, xyz, feats, cost_volume, flow = None, iters=3):
        '''
        feats: B C1 N
        cost_volume: B C2 N
        flow: B 3 N
        ...
        for iter in range(iters):
            feats_out = torch.cat([feats_out, cost_volume], dim = 1)
            for _, pointconv in enumerate(self.pointconv_list):
                feats_out = pointconv(xyz, feats_out)

            for conv in self.mlp_convs:
                feats_out = conv(feats_out)

            flow_local = self.fc(feats_out).clamp(self.clamp[0], self.clamp[1]) 
