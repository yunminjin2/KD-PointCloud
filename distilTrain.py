...
def main():
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            t_model.eval()
            with torch.no_grad():
                t_pred_flows, t_fps_pc1_idxs, _, _, _ = t_model(pos1, pos2, norm1, norm2)
            st_model.train()         
...
def analyzing():
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        with torch.no_grad():
            t_pred_flows, t_fps_pc1_idxs, _, _, _ = t_model(pos1, pos2, norm1, norm2)
        st_model.train()
        pred_flows, fps_pc1_idxs, _, _, _ = st_model(pos1, pos2, norm1, norm2)

        # flow_0 = flow.cpu().detach().numpy()
        # pred_flow_0 = t_pred_flows[0].permute(0, 2, 1).cpu().detach().numpy()
        # for i in range(0, len(flow[0])):
        # print(pred_flow_0.shape)
        # print(flow_0.shape)
        # print(pred_flow_0)
        # print(flow_0)
        # print(pred_flow_0 - flow_0)

        print(multiScaleLoss(t_pred_flows, flow, t_fps_pc1_idxs))
