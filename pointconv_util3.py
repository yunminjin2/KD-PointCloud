...
class BottleNeck(nn.Module):
    def __init__(self, in_channels, mid_channel, out_channel, kernel_size=1, stride=1, padding=0, use_leaky=True, bn=use_bn):
        super(BottleNeck, self).__init__()
        self.bottleneck = nn.Sequential(
            nn.Conv1d(in_channels=in_channels, out_channels=mid_channel, kernel_size=1),
            nn.Conv1d(in_channels=mid_channel, out_channels=mid_channel, kernel_size=3, padding=1, bias=False),
            nn.Conv1d(in_channels=mid_channel, out_channels=out_channel, kernel_size=1),
        )
        self.depthwiseConv = ConvBNReLU(in_channels, out_channel)
        self.relu = nn.ReLU()

    def forward(self, x):
        fx = self.bottleneck(x)  # F(x)
        fy = self.depthwiseConv(x)
        out = fx + x + fy  # F(x) + x
        out = self.relu(out)
        return out
...
class PointConvNonLinear(nn.Module):
    def __init__(self, npoint, nsample, in_channel, out_channel, weightnet = 16, bn = use_bn, use_leaky = True):
        super(PointConvNonLinear, self).__init__()
        self.npoint = npoint
        self.bn = bn
        self.nsample = nsample
        self.weightnet = WeightNet(3, weightnet)
        ...
        if bn:
            self.bn_linear = nn.BatchNorm1d(out_channel)
        self.conv = nn.Conv2d(in_channel, in_channel, kernel_size=1, groups=in_channel)
        self.pointwise = nn.Conv2d(in_channel, out_channel//weightnet, kernel_size=1)
    ...
    def forward(self, xyz, points):
        ...
        weights = self.weightnet(grouped_xyz)
        # B, N, S, C
        new_points = torch.matmul(input=new_points.permute(0, 1, 3, 2), other = weights.permute(0, 3, 2, 1)).permute(0, 2, 3, 1)

        new_points = self.conv(new_points)
        new_points = self.pointwise(new_points)
        new_points = new_points.view(B, self.npoint, -1)
