...
__all__ = ['FlyingThings3DSubsetMin']


class FlyingThings3DSubsetMin(data.Dataset):
    """
    Args:
    ...
    def __init__(self,
                 ...
                 data_root,
                 full = True):
        self.root = osp.join(data_root, 'FlyingThings3D_subset_processed_min')
        self.train = train
    ...
    def make_dataset(self, full):
        ...
        try:
            if self.train:
                assert (len(useful_paths) == 4504)
            else:
                assert (len(useful_paths) == 451)
