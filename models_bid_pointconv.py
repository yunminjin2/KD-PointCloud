...
class PointConvBidirection2(nn.Module):
    def __init__(self):
        super(PointConvBidirection2, self).__init__()
...
class PointConvBidStudentModel2(nn.Module):
    def __init__(self):
        ...
        self.level0 = Conv1d(3, 32)
        self.level0_0 = BottleNeck(32, 16, 32)
        self.level0_1 = Conv1d(32, 64)
        self.cross0 = CrossLayer(flow_nei, 32 + 32 , [32, 32], [32, 32]) # nsample=32, in_channel=32+32, mlp1=[32, 32], mlp2=[32,32] 
        self.flow0 = SceneFlowEstimatorResidual(32 + 64, 32) # feat_ch = 32+64, cost_ch=32

        self.level1 = PointConvD(2048, feat_nei, 64 + 3, 64)
        self.level1_0 = BottleNeck(64, 16, 64)
        self.level1_1 = Conv1d(64, 64)
        self.cross1 = CrossLayer(flow_nei, 64 + 32, [64, 64], [64, 64])
        self.flow1 = SceneFlowEstimatorResidual(64 + 64, 64)
...
def hintLoss(feat1, t_feat1, t_fps_idx):
    return 0 

def loss_fn_ht(outputs, feat1s, fps_idxs1, fps_idxs2, gt_flow, teacher_outputs, t_feat1s, teacher_fps_idxs, gamma,  alpha=[0.02, 0.04, 0.08, 0.16]):
    KD_loss = torch.zeros(1).cuda()
    ...
    Hint_loss = ((feat1s[0]-t_feat1s[0])**2)/2
