...
class PointConvBidStudentModel(nn.Module):
    def __init__(self):
        ...
        self.cross2 = CrossLayer(flow_nei, 64 + 64, [64, 64], [64, 64])
        ...
        self.cross3 = CrossLayer(flow_nei, 128 + 64, [128, 128], [128, 128])
        self.flow3 = SceneFlowEstimatorResidual(128, 128, flow_ch=0)

        self.level4 = PointConvD(64, feat_nei, 256 + 3, 128)

        self.deconv4_3 = Conv1d(128, 64)
        self.deconv3_2 = Conv1d(128, 64)
...
class PointConvBidStudentModel2(nn.Module):
    def __init__(self):
        super(PointConvBidStudentModel2, self).__init__()
...
def attentiveImitatinoLoss(outputs, fps_idxs, gt_flow, teacher_outputs, teacher_fps_idxs, t_history, gamma, alpha=[0.02, 0.04, 0.08, 0.16]):
    KD_loss =torch.zeros(1).cuda()
    ...
    KD_loss += gamma*(1-sigma)*(loss_SG) + (1-gamma)*sigma*(loss_ST)
